...
fn main(){
    ...
    let mut string: Vec<char> = "abcdefgh".chars().collect();
    ...
    for i in instructions {
        if i[1] == "position" && i[0] == "swap" {
            ...
            let mut steps = string.iter().position(|&x| x == i[6].parse::<char>().unwrap()).unwrap();
            if steps > 3 {
                steps += 1;
            }
            steps += 1;
            ...
            let moveind = i[2].parse::<usize>().unwrap();
            let moveto = i[5].parse::<usize>().unwrap();
